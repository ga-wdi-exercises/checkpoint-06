...
```text
the data-ng-click will be able to validate, the validator ignores anything that comes after the data keyword
```
...
```text
Answer [A] makes the best use of the ng-app. It is best practice to always bootstrap the ng-app tag in the html tag. it designates the root element so capturing the entire page is better than just the small portion
```
...
```text
ui-sref is an angular directive that binds a link (similiar to anchor tag) to a state. It allows for URL's or relative paths for navigating
```
...
$.ajax({
  type: 'POST',
  data: {artist: {title: "A new post", body: "doesn't need much"}},
  dataType: 'json',
  url: "/artists"
}).done((response) =>  {
  console.log(response);
