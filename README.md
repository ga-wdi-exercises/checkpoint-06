...
```text
the two are functionally identical, the only difference is that if you use a html validator ng-click will freak it out while data-ng-click wont
```
...
```text
A is the best answer because it makes the scope of the app the entire page rather than just the head or a particular div
```
...
```text
ui-sref can be used to create a link to a different view within the app (by way of a particular state and maybe an object passed to ui-sref)
```
